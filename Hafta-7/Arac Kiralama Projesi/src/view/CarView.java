...
//**************************************************************************************************************
        ...
        for (Model model : this.modelManager.findAll()){
            this.cmb_model.addItem(model.getComboItem()); // model sınıfından combo item urettirirdim key ve value degerleri
            //alıyor carview a
        }

//**************************************************************************************************************
// car id si varsa bu bir update işlemidir. burada kolonları dolu gelmesini saglıyoruz
        if (this.car.getId() != 0) {
...
//**************************************************************************************************************
        //km ve plate  bos mu diye soruyorum digelerini doldurmustum
        this.btn_car_save.addActionListener(e -> {
            if (Helper.isFieldListEmpty(new JTextField[]{this.fld_km, this.fld_plate})) {
                ...
                //combo dan seci item i alıyrum ve altta degiştiriyorum ya da save yapıyorum
